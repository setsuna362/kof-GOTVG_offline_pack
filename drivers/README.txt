...
該 romset 的 ROM 配置(manifest.json 裡每套的 host 欄)。26 套裡有 19 套
借得到現成槽位,直接改名即可。

有七套借不到 —— FBNeo 裡沒有配置相符的 romset,必須自行加驅動並重建 FBNeo:

  kof96aeg.c    給 kof96ae 用
  kof99aeg.c    給 kof99ae 用
  kof2003tg.c   給 kof2003t 用
  kof2k2g.c     給 kf2k2pp / kof2002kai / kof2002p33 / kof2002prsp 四套共用
...
kf2k2pp / kof2002kai / kof2002p33 / kof2002prsp
  ...
  四套共用同一個槽位,所以驅動表裡的 p1 / p2d / c1d / c2d 只對得上其中
...
kof99ae
  官方的 kof99ae 是 12 顆 C(96MB)加一顆 p3,超過 CHA512Y 的 8x8MB 上限,
  做不成實體卡。本包收的是砍到 8 顆 C(64MB,剛好貼齊)、且不含 p3 的版本
  —— 來源檔裡 c9~c12 本來就是整片 0x00,原本在那裡的圖素已搬進 c7/c8。
...
編好之後把產出的 zip 改名成對應的載入名稱再載入:
  ...
  out/kof2002p33.zip   -> kof2k2g.zip
  out/kof2002prsp.zip  -> kof2k2g.zip
  out/kof99ae.zip      -> kof99aeg.zip
